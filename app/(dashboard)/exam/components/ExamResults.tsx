    ...
    <div className="min-h-screen bg-background text-foreground py-10 px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-8 text-center space-y-4">
          ...
          <div>
            <h1 className="text-xl font-semibold text-foreground">{grade}</h1>
            <p className="text-sm text-muted-foreground mt-1">{config.subjectName} · {config.topicName}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 pt-2">
            ...
            <div className="bg-background border border-border rounded-xl p-3">
              ...
              <p className="text-lg font-bold text-foreground">{formatTime(timeTakenSec)}</p>
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={onRetry}
              className="flex-1 flex items-center justify-center gap-2 bg-accent hover:bg-accent/80 text-foreground text-sm font-medium py-2.5 rounded-xl transition-colors"
            >
        ...
        <h2 className="text-xs font-mono text-muted-foreground tracking-widest">QUESTION REVIEW</h2>
        ...
        {problems.map((problem, idx) => {
          ...
          return (
            <div key={problem.id} className={`bg-card border rounded-2xl p-5 space-y-4 ${isCorrect ? "border-emerald-400/30" : "border-red-400/30"}`}>
                ...
                {(["a", "b", "c", "d"] as const).map(key => {
                  ...
                  return (
                    <div key={key} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border text-sm ${
                      isCorrectOpt                    ? "border-emerald-400 bg-emerald-400/10 text-emerald-400"
                      : isSelected && !isCorrectOpt   ? "border-red-400 bg-red-400/10 text-red-400"
              ...
              {problem.explanation && (
                <div className="pl-10 bg-background border border-border rounded-xl px-4 py-3 space-y-1">
