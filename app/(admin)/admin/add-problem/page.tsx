...
/* ── TikZ extraction ─────────────────────────────────────────────────────── */
// Extract \begin{tikzpicture}...\end{tikzpicture} blocks before KaTeX sees them
function extractTikz(text: string): string {
  return text
      // Strip \begin{center}...\end{center} wrappers first, keep inner content
      .replace(/\\begin\{center\}([\s\S]*?)\\end\{center\}/g, (_, inner) => inner.trim())
      // Strip \centering command
      .replace(/\\centering\b/g, '')
      // Extract tikzpicture blocks into markers
      .replace(
          /\\begin\{tikzpicture\}[\s\S]*?\\end\{tikzpicture\}/g,
          (match) => `%%TIKZ:${encodeURIComponent(match)}%%`
      )
      // Also handle circuitikz
      .replace(
...
/* ── MathText: renders mixed plain text + LaTeX (all delimiter styles) ───── */
...
function MathText({ text, className = "" }: { text: string; className?: string }) {
  if (!text) return <span className={`text-zinc-600 italic ${className}`}>empty</span>;
  ...
  return (
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="text-xs font-mono text-zinc-500 tracking-wider uppercase">{label}</label>
...
/* ── Textarea with live LaTeX preview ────────────────────────────────────── */
...
export default function ProblemEditor() {
  ...
  const handleSubmit = async () => {
    ...
    for (const key of required) {
      if (!form[key]) {
        setStatus("error");
        setErrMsg(`Please fill in: ${key.replace("_", " ")}`);
        return;
      }
    }
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <Field label="Access">
                <Select
                    value={form.is_free ? "free" : "pro"}
                    onChange={v => set("is_free", v === "free")}
                    options={[
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
            ...
            <LaTeXField
                ...
                placeholder="A nudge shown on request (Pro feature)..."
                rows={2}
