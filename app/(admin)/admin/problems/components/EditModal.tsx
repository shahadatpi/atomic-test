...
function splitExplanation(expl: string): [string, string, string, string] {
  const result: [string, string, string, string] = ["", "", "", ""];
  if (!expl) return result;
  const pattern = new RegExp(`(${CQ_LABELS.map(l => `${l}[).]`).join("|")})`, "g");
  const tokens  = expl.split(pattern).map(s => s.trim()).filter(Boolean);
  if (tokens.length <= 1) { result[0] = expl.trim(); return result; }
  let current = -1;
  for (const token of tokens) {
    const idx = CQ_LABELS.findIndex(l => token === `${l})` || token === `${l}.`);
    if (idx !== -1) { current = idx; continue; }
    if (current >= 0)
      result[current] = (result[current] ? result[current] + " " : "") + token;
  }
  return result;
}

function joinExplanations(parts: [string, string, string, string]): string {
  return parts
    .map((p, i) => p.trim() ? `${CQ_LABELS[i]}) ${p.trim()}` : "")
    .filter(Boolean)
    .join("\n\n");
}
...
/** Inline create/rename input for topic or subtopic */
function InlineCreate({
  mode, value, onChange, onSave, onCancel, saving, error, placeholder,
}: {
  mode:        "create" | "rename";
  value:       string;
    ...
    <div className="space-y-1.5">
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-4 py-2 bg-violet-500 hover:bg-violet-400 disabled:opacity-50
                     text-white text-xs rounded-xl font-semibold transition-colors whitespace-nowrap"
        >
...
function Tab({
  ...
  id: TabId; active: boolean; icon: React.ElementType; label: string; onClick: () => void;
}) {
  return (
    <button
...
export default function EditModal({
  ...
  const [cqExpls,       setCqExpls]       = useState<[string,string,string,string]>(() =>
    splitExplanation(problem.explanation ?? "")
  );
  ...
  const saveTopicName = async () => {
    ...
    const { error } = await supabase.from("topics")
      .update({ name: topicNameVal.trim() }).eq("id", topicId);
  ...
  const saveSubtopicName = async () => {
    ...
    const { error } = await supabase.from("subtopics")
      .update({ name: subtopicNameVal.trim() }).eq("id", subtopicId);
  ...
  const handleSave = async () => {
    ...
    const finalExplanation = isMcqType
      ? explanation.trim() || null
      : joinExplanations(cqExpls) || null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="w-full max-w-5xl bg-zinc-900 border border-zinc-700/80 rounded-2xl shadow-2xl
        ...
        <div className="flex items-center gap-1.5 px-6 py-3 border-b border-zinc-800 shrink-0 overflow-x-auto">
          <Tab id="content"        active={activeTab === "content"}
               icon={AlignLeft}    label="Content"        onClick={() => setActiveTab("content")} />
          <Tab id="classification" active={activeTab === "classification"}
               icon={CheckSquare}  label="Classification" onClick={() => setActiveTab("classification")} />
          <Tab id="meta"           active={activeTab === "meta"}
               icon={Settings}     label="Meta & Tags"    onClick={() => setActiveTab("meta")} />
          {/* type badge */}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5">
          ...
          {activeTab === "content" && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <LaTeXField
                  label="" value={question} onChange={setQuestion} rows={4}
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <p className="text-[11px] text-amber-400/70 font-mono">
                      Click letter → set correct answer
                    </p>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {(["a","b","c","d"] as const).map((key, i) => {
                      ...
                      return (
                        <div key={key} className="border border-zinc-800 rounded-2xl overflow-hidden
                                                   bg-zinc-950/30">
                          {/* Label row */}
                          ...
                          <div className="px-4 pb-3">
                            <LaTeXField
                              label="" value={options[key] ?? ""} rows={2}
                          ...
                          <div className="border-t border-zinc-800/60">
                            <button type="button"
                              ...
                              <span className={cqExpls[i] ? "text-emerald-400" : "text-amber-400/70"}>
                                {CQ_LABELS[i]} — সমাধান
                                {cqExpls[i] && " ✓"}
                              </span>
                            ...
                            {isOpen && (
                              <div className="px-4 pb-4 pt-1 bg-zinc-950/50">
                                <LaTeXField
                                  label="" value={cqExpls[i]} rows={3}
                                  placeholder={`${CQ_LABELS[i]} সমাধান লিখুন…`}
                                  onChange={v => setCqExpls(prev => {
              ...
              {isMcqType && (
                <div>
                  ...
                  <LaTeXField
                    label="" value={explanation} onChange={setExplanation}
          ...
          {activeTab === "classification" && (
            <div className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    {topicId && topicMode === "select" && (
                      <button type="button"
                        onClick={() => { setTopicNameVal(allTopics.find(t => t.id === topicId)?.name ?? ""); setTopicMode("rename"); setTopicErr(""); }}
                        className="text-[11px] text-zinc-500 hover:text-violet-400 transition-colors font-mono">
                        ✎ rename
                      </button>
                    )}
                    {topicMode === "select" && (
                      <button type="button"
                        onClick={() => { setTopicNameVal(""); setTopicMode("create"); setTopicErr(""); }}
                    ...
                    {topicMode !== "select" && (
                      <button type="button"
                        onClick={() => { setTopicMode("select"); setTopicErr(""); }}
                ...
                {topicMode !== "select" ? (
                  <InlineCreate
                    mode={topicMode} value={topicNameVal} onChange={setTopicNameVal}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    {subtopicId && subtopicMode === "select" && (
                      <button type="button"
                        onClick={() => { setSubtopicNameVal(allSubtopics.find(s => s.id === subtopicId)?.name ?? ""); setSubtopicMode("rename"); setSubtopicErr(""); }}
                        className="text-[11px] text-zinc-500 hover:text-violet-400 transition-colors font-mono">
                        ✎ rename
                      </button>
                    )}
                    {subtopicMode === "select" && (
                      <button type="button"
                        onClick={() => { setSubtopicNameVal(""); setSubtopicMode("create"); setSubtopicErr(""); }}
                        disabled={!topicId}
                        className="text-[11px] text-emerald-500 hover:text-emerald-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors font-mono">
                        + new
                      </button>
                    )}
                    {subtopicMode !== "select" && (
                      <button type="button"
                        onClick={() => { setSubtopicMode("select"); setSubtopicErr(""); }}
                        className="text-[11px] text-zinc-600 hover:text-zinc-400 transition-colors font-mono">
                        ✕ cancel
                      </button>
                    )}
                ...
                {subtopicMode !== "select" ? (
                  <InlineCreate
                    mode={subtopicMode} value={subtopicNameVal} onChange={setSubtopicNameVal}
              ...
              <div>
                ...
                {pickerInst && (
                  <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 space-y-3 mb-3">
                    <p className="text-[11px] text-zinc-500 font-mono">Select year for {pickerInst}:</p>
                    ...
                    <button type="button"
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-violet-500
                                 hover:bg-violet-400 text-white text-xs font-semibold transition-all">
          ...
          {/* Error */}
          {error && (
            <div className="mt-4 text-xs text-red-400 bg-red-400/10 border border-red-400/20
