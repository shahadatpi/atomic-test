...
function validateEmail(email: string): { valid: boolean; reason?: string } {
  ...
  // Must have proper TLD (at least 2 chars)
  const tld = email.split(".").pop() ?? ""
  if (tld.length < 2) return { valid: false, reason: "Invalid domain" }
  // No consecutive dots
  if (email.includes("..")) return { valid: false, reason: "Invalid email format" }
  // Disposable domain blocklist (common ones)
  ...
  if (blocked.includes(domain)) return { valid: false, reason: "Disposable emails not allowed" }
  return { valid: true }
}
...
export function SignupForm({
  ...
  const canSubmit = !loading && name.trim() && validateEmail(email).valid && password.length >= 4 && password === confirmPassword;
        ...
        <div className="w-full max-w-4xl grid md:grid-cols-2 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl">
          ...
          <div className="bg-zinc-900 px-8 py-10 flex flex-col justify-center space-y-6">
            ...
            <div className="fade-up d3 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  id="name" type="text" placeholder="Mofiz Uddin"
              ...
              {/* Password strength + confirm match hint */}
              {password.length > 0 && (
            ...
            <div className="fade-up d4 space-y-3">
              <button
                type="button" onClick={handleSignup} disabled={!canSubmit}
          ...
          <div className="relative hidden md:flex flex-col items-center justify-center bg-zinc-950 border-l border-zinc-800 overflow-hidden p-10 gap-8">
            ...
            <div className="relative z-10">
              <Link href="/">
                <Image src="/atomic-test-logo.png" width={120} height={120} alt="AtomicTest"
            ...
            <div className="relative z-10 flex flex-wrap justify-center gap-2">
              {["LaTeX rendering", "Instant feedback", "Progress tracking", "Free & Pro plans"].map(f => (
                <span key={f} className="text-xs text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 px-3 py-1 rounded-full">
                  ✦ {f}
                </span>
              ))}
            </div>
